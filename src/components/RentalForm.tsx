...
export function toFormValues(option?: RentalOption): RentalFormValues {
  return {
    ...
    estimatedBills: option?.estimatedBills ?? 80,
    deposit: option?.deposit ?? 700,
    agencyFee: option?.agencyFee ?? 0,
    locationLabel: option?.locationLabel ?? "",
    commuteMinutes: option?.commuteMinutes,
    size: option?.size,
    furnished: option?.furnished ?? true,
    bathroomType: option?.bathroomType ?? "shared",
    contractAvailable: option?.contractAvailable ?? true,
    availableDate: option?.availableDate ?? "",
    status: option?.status ?? "new",
    notes: option?.notes ?? "",
    photoUri: option?.photoUri ?? "",
    locationRating: option?.locationRating ?? 7,
    roomQualityRating: option?.roomQualityRating ?? 7,
    personalFeelingRating: option?.personalFeelingRating ?? 7,
  };
}
